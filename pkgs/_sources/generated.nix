# This file was generated by nvfetcher, please do not modify it manually.
{
  fetchgit,
  fetchurl,
  fetchFromGitHub,
  dockerTools,
}:
{
  Ech0 = {
    pname = "Ech0";
    version = "v4.1.6";
    src = fetchFromGitHub {
      owner = "lin-snow";
      repo = "Ech0";
      rev = "v4.1.6";
      fetchSubmodules = false;
      sha256 = "sha256-bs/2wBYpoImHF/aGllkti6lEv7cuPylolL2+SbEDGoA=";
    };
  };
  Ech0-frontend = {
    pname = "Ech0-frontend";
    version = "v4.1.6";
    src = fetchFromGitHub {
      owner = "lin-snow";
      repo = "Ech0";
      rev = "v4.1.6";
      fetchSubmodules = false;
      sha256 = "sha256-bs/2wBYpoImHF/aGllkti6lEv7cuPylolL2+SbEDGoA=";
    };
  };
  einat = {
    pname = "einat";
    version = "v0.1.10";
    src = fetchFromGitHub {
      owner = "EHfive";
      repo = "einat-ebpf";
      rev = "v0.1.10";
      fetchSubmodules = false;
      sha256 = "sha256-P0Q8DdeEns5xcALj5vIyubc1AE073nozbNoTzRso37w=";
    };
    extract = {
      "Cargo.lock" = ./. + "/sha256-P0Q8DdeEns5xcALj5vIyubc1AE073nozbNoTzRso37w=/Cargo.lock";
    };
  };
  fcitx5-pinyin-zenith = {
    pname = "fcitx5-pinyin-zenith";
    version = "v2026.03.02";
    src = fetchurl {
      url = "https://github.com/ZenQy/scel2dict/releases/download/v2026.03.02/zenith.dict";
      sha256 = "sha256-BwlhWSJWPkqHfOm3whLtdEAs0AXlskW4rA8q8p4aTj0=";
    };
  };
  linux-flippy-612 = {
    pname = "linux-flippy-612";
    version = "a87a2290e25fb0fe1cd5f18f91dbf2108e236f6d";
    src = fetchFromGitHub {
      owner = "unifreq";
      repo = "linux-6.12.y";
      rev = "a87a2290e25fb0fe1cd5f18f91dbf2108e236f6d";
      fetchSubmodules = false;
      sha256 = "sha256-82d1Wbqbsnvt6FyuJYrxGohggJv+z4bCkdUQLfrlANY=";
    };
    extract = {
      Makefile = ./. + "/sha256-82d1Wbqbsnvt6FyuJYrxGohggJv+z4bCkdUQLfrlANY=/Makefile";
    };date = "2026-03-20";
  };
  linux-flippy-618 = {
    pname = "linux-flippy-618";
    version = "v6.18.19";
    src = fetchFromGitHub {
      owner = "unifreq";
      repo = "linux-6.18.y";
      rev = "v6.18.19";
      fetchSubmodules = false;
      sha256 = "sha256-Iq96JKyD6fZ9yyVsts2nArqX7nQN1ZMjCAxgzdlO61s=";
    };
  };
  vitejs = {
    pname = "vitejs";
    version = "v8.0.1";
    src = fetchFromGitHub {
      owner = "vitejs";
      repo = "vite";
      rev = "v8.0.1";
      fetchSubmodules = false;
      sha256 = "sha256-deR1/LHvWetzuu2wBQEo6LodrevhhLH0gvTLpnoqXuk=";
    };
  };
  wallpapers = {
    pname = "wallpapers";
    version = "v2026.03.03";
    src = fetchurl {
      url = "https://github.com/ZenQy/wallpaper/releases/download/v2026.03.03/wallpaper.tar.gz";
      sha256 = "sha256-9G/vXsvbpOuQRRbYMsUySWOTLzpMmmiZGrhHpv9kkdc=";
    };
  };
}
