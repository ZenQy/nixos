  ...
  outputs = { self, nixpkgs, secrets, impermanence }:
    ...
    in {
      overlays.default = final: prev:
        (nixpkgs.lib.composeExtensions this.overlay (import ./pkgs/override.nix)
          final prev);

      nixosModules.desktop = { pkgs, ... }: {
        imports = [ desktop impermanence.nixosModule ];
      };
      nixosModules.server = { pkgs, ... }: { imports = [ server ]; };
      ...
      in builtins.mapAttrs (host: module:
        nixpkgs.lib.nixosSystem {
          system = "x86_64-linux";
          specialArgs = { secrets = import (secrets + "/secrets.nix"); };
          modules = [
            ./hosts/${host}
            {
